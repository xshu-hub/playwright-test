...
class TestCart:
    ...
    @pytest.mark.parametrize(
        "product_index", [0, 1, 2], ids=["第一个商品", "第二个商品", "第三个商品"]
    )
    def test_add_specific_product_to_cart(
        self, logged_in_inventory_page: InventoryPage, cart_page: CartPage, product_index: int
    ):
        """
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.cart
    def test_view_cart_items(self, logged_in_inventory_page: InventoryPage, cart_page: CartPage):
        """
        测试查看购物车中的商品详情
        """
        inventory_page = logged_in_inventory_page
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @pytest.mark.cart
    def test_continue_shopping(self, logged_in_inventory_page: InventoryPage, cart_page: CartPage):
        """
        测试从购物车返回继续购物
        """
        inventory_page = logged_in_inventory_page
...
class TestCheckout:
    ...
    @pytest.mark.smoke
    @pytest.mark.cart
    def test_checkout_flow(self, logged_in_inventory_page: InventoryPage, cart_page: CartPage):
        """
        测试完整的结账流程（使用测试数据）
        """
        inventory_page = logged_in_inventory_page
